...
class Tubit:
    ...
    def setup_ui(self):
        # Create main canvas and scrollbar for entire window
        ...
        # Pack main scrollbar and canvas
        main_scrollbar.pack(side="right", fill="y")
        main_canvas.pack(side="left", fill="both", expand=True)
        
        # Add padding frame inside main container
        padded_frame = tk.Frame(main_container, bg=self.bg_color)
        padded_frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=15)
        
        # Store references
        self.main_canvas = main_canvas
        
        # Enable mouse wheel scrolling for entire window
        def _on_main_mousewheel(event):
            main_canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        
        def _on_main_mousewheel_linux(event):
            if event.num == 4:
        ...
        self.root.bind_all("<Button-4>", _on_main_mousewheel_linux)
        self.root.bind_all("<Button-5>", _on_main_mousewheel_linux)
        ...
        # Quality Selection Section - Grid Layout with CENTER alignment
        ...
        # CENTERED CONTAINER for formats
        self.scrollable_container = tk.Frame(canvas, bg=self.card_color)
        self.scrollable_frame = tk.Frame(self.scrollable_container, bg=self.card_color)
        self.scrollable_frame.pack(expand=True)  # This centers the grid
        ...
        canvas.create_window((0, 0), window=self.scrollable_container, anchor="n")
        ...
        canvas.bind("<Enter>", lambda e: None)
        ...
        # Progress bar section with centered header
        progress_header_frame = tk.Frame(progress_section, bg=self.card_color)
        progress_header_frame.pack(fill=tk.X, pady=(0, 10))
        
        progress_label = tk.Label(progress_header_frame, text="Download Progress",
                                font=('Arial', 12, 'bold'),
                                bg=self.card_color, fg=self.text_color)
        progress_label.pack()
    ...
    def create_format_grid(self):
        """Create format selection buttons in a CENTERED grid layout with fixed 2 columns"""
        ...
        # Fixed 2 columns for non-resizable window
        columns = 2
        ...
        for i, format_info in enumerate(self.all_formats):
            ...
            button_frame.grid(row=current_row, column=current_col, padx=8, pady=8, sticky="nsew")
            
            # Simplified format details for display
            ...
            btn = tk.Button(button_frame, text=display_text,
                          ...
                          padx=20, pady=15,
                          justify=tk.CENTER,
                          wraplength=180, width=18, height=4)
            btn.pack(fill=tk.BOTH, expand=True)
            
            # Restore selection if this was previously selected
            if self.selected_format and self.selected_format['format_id'] == format_info['format_id']:
